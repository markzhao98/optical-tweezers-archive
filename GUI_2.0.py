...
class App:
    ...
    def update(self):
        ...
        if mmc.getRemainingImageCount() > 0:
            ...
            prephoto = img.view(dtype=np.uint8).reshape(img.shape[0],
                               img.shape[1], 2)[..., :4]
        else:
            prephoto = cv2.imread('blank.png')
        
        pil = PIL.Image.fromarray(prephoto)
        
        pil = pil.resize((696,520))

        self.canvas.photo = PIL.ImageTk.PhotoImage(pil)
